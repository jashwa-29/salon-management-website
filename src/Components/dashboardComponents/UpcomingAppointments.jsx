...
import { Calendar, Plus, Edit, Clock, CheckCircle } from 'lucide-react';

function UpcomingAppointments({ appointments = [], onStatusUpdate, onViewAll }) {
    ...
    <div className="bg-black rounded-xl border border-gray-800 p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          type="button"
          onClick={() => onViewAll?.()}
          className="text-yellow-500 text-sm hover:underline flex items-center space-x-2"
        >
          <span>View All</span>
      ...
      {appointments.length > 0 ? (
        ...
        <div className="text-center py-8">
          ...
          <button className="mt-4 text-yellow-500 hover:underline flex items-center justify-center mx-auto">
            <Plus className="w-4 h-4 mr-1" /> Schedule one now
          </button>
        </div>
      )}
    </div>
